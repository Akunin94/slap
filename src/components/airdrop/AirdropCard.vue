<template>
  <div class="sl-airdrop-card">
    <div v-if="data.icon" class="sl-airdrop-card__icon">
      <component class="w-[52px] h-[52px]" :is="iconComponent(data.icon)" />
    </div>
    <div class="sl-airdrop-card__info">
      <div class="sl-airdrop-card__title">{{ data.title }}</div>
      <div v-if="data.amount" class="sl-airdrop-card__amount">
        {{ data.amount }}
      </div>
      <div v-if="data.reward" class="sl-airdrop-card__reward">
        <IconCoin class="mr-2 w-[24px]" /> {{ data.reward }}
        <span class="sl-airdrop-card__lvl" v-if="data.lvl"
          >{{ data.lvl }} lvl</span
        >
      </div>
    </div>
    <div v-if="data.done">
      <IconCheck class="w-[24px]" />
    </div>
    <div v-else>
      <IconArrowRight class="w-[24px]" />
    </div>
  </div>
</template>
