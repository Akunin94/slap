...
export default {
  ...
  methods: {
    ...
    persistClicks() {
      ...
      window.Telegram.WebApp.CloudStorage.setItem('clicks_count', this.balance, (error, success) => {
        if (error) {
    ...
    fetchClicks() {
      try {
        window.Telegram.WebApp.CloudStorage.getItem('clicks_count', (error, value) => {
          if (error) {
            console.error(error);
          } else {
            this.balance = +value;
